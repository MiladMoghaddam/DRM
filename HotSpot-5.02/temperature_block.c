...
void dump_temp_block(block_model_t *model, double *temp, char *file)
{
	...
	long double alpha, beta, meanT, meanN, MTTF, randomX1[flp->n_units], randomX2[flp->n_units], min;
	...
	for (j=0; j < 100000; j++) {
		...
		for (i=0; i < flp->n_units; i++) {
			...
			if(j == 0) fprintf(fp, "%s\t%.2f\n", flp->units[i].name, temp[i]);

			// Calculate MTTF due to TDDB
		
			MTTF = 2.406090e23*(long double)pow(((long double)1/(long double)2), (long double)78+(long double)0.081*temp[i])*(long double)exp((0.759 -66.8/temp[i] - 8.37e-4*temp[i])/(8.617e-5*temp[i]));
			MTTF = MTTF/flp->units[i].width;
			MTTF = MTTF/flp->units[i].height;		
			alpha = MTTF/exp(lgammal(1+(1/beta)));
			// Calculate Weibull random variable (TDDB) here
			randomX1[i] = alpha*pow(-log(1-((long double)rand()/(long double)RAND_MAX)),(1/beta));

			// Calculate MTTF due to NBTI

			MTTF = 0.9621e-5*pow((long double)((log(1.6328/(1+2*exp(0.07377/(8.617e-5*temp[i]))))-log(1.6328/(1+2*exp(0.07377/(8.617e-5*temp[i])))-0.01))*(temp[i]/exp(-0.06852/(8.617e-5*temp[i])))), (long double)((long double)1/(long double)0.3));
			MTTF = MTTF/flp->units[i].width;
			MTTF = MTTF/flp->units[i].height;			
			alpha = MTTF/exp(lgammal(1+(1/beta)));
			// Calculate Weibull random variable (NBTI) here
			randomX2[i] = alpha*pow(-log(1-((long double)rand()/(long double)RAND_MAX)),(1/beta));
		}
		...
		meanN = meanN + (min/100000);
	}
	//Print final mean
    // do not do it here anymore because the entire MTTF calculation
    // is done now in REST as a separate tool;
    //fprintf(fp, "Whole system MTTF: %Le\n", meanN);
