...
int main(int argc, char *argv[])
{
    char *fb;
    char *outfile_name;
    bool infile_specified = false, outfile_specified = false;
    int plevel  = 2;
    opt_for_clk = true;
    ...
    p1->parse(fb);
    Processor proc(p1);
    proc.displayEnergy(outfile_name, 2, plevel);
    delete p1;
    return 0;
}
...
void print_usage(char * argv0)
{
    ...
    cerr << "mcpat -infile <input file name> "<< endl; 
    cerr << "  -outfile <output file name> "<< endl;
    cerr << "  -print_level < level of details 0~5 > "<< endl; 
    cerr << "  -opt_for_clk < 0 (optimize for ED^2P only)/1 (optimzed for target clock rate)>"<< endl;
