...
for i in range(0,num_cores):
	print "%s %0.2f K ------------ (%0.2f C)"%(i,float(TempInArea[i]/areaSum[i]),float(TempInArea[i]/areaSum[i])-273.00)
	TEMPERATURE.write("%s %0.2f K ------------ (%0.2f C)\n"%(i,float(TempInArea[i]/areaSum[i]),float(TempInArea[i]/areaSum[i])-273.00))
